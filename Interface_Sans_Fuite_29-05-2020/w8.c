...
/*---------------------------- Morpion ------------------------------*/
			/*---------------------------- Variables ---------------------------*/
				...
				char lignegagn = NULL; /* lignegagn nous permettra de savoir si l'ordinateur peut finir la partie ou non. */
			...
			/*---------------------------- Prototype ---------------------------*/
				...
				void click_morpion(SDL_Renderer *renderer, SDL_Window *window, SDL_Event *event1, int restart,int ** Grille,int *Tjoue,SDL_Surface *image_croix,SDL_Texture *texture_croix,SDL_Surface *image_rond,SDL_Texture *texture_rond);
				//Selection morpion
				void affiche_point_morpion(SDL_Renderer *renderer, SDL_Window *window, SDL_Event *event1);
				//Affichage croix
				void croix_morpion(SDL_Renderer *renderer, SDL_Window *window, SDL_Event *event1, int restart,SDL_Surface *image_croix,SDL_Texture *texture_croix,SDL_Surface *image_rond,SDL_Texture *texture_rond);
										...
										while(SDL_PollEvent(&event_morpion)){
											switch(event_morpion.type){
													...
													case SDL_MOUSEBUTTONDOWN:
														...
														/* click_morpion(renderer, window, &event,0,Grille, &Tjoue,image_croix,texture_croix,image_rond,texture_rond ); /* on teste si le clique correspond à un endroit utile */
...
void croix_morpion(SDL_Renderer *renderer, SDL_Window *window, SDL_Event *event1, int restart,SDL_Surface *image_croix,SDL_Texture *texture_croix,SDL_Surface *image_rond,SDL_Texture *texture_rond){/* permet le reload de la grille de morpion */
