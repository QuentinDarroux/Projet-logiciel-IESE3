...
void afficher_symbole(SDL_Renderer *renderer, SDL_Window *window,int posy , int posx, int* Tjoue); /* fonction qui affichera une croix ou un rond au bon endroit */
void verification(int y,int x,int **Grille,SDL_Renderer *renderer, SDL_Window *window,int* Tjoue); /* fonction qui vérifie l'état de la grille , si le prochain coup est gaggant et ou si la partie et finie. */
int testGrilleremplie(int ** Grille); /* on regarde si la grille est totalement remplie */
void ordiJoue(int X, int Y, char lignegagn, char lignedef, int ** Grille,SDL_Renderer *renderer, SDL_Window *window,int* Tjoue); /* l'ordinateur joue */
/* detection de la case selectionnee */
void click_morpion(SDL_Renderer *renderer, SDL_Window *window, SDL_Event *event1, int restart,int ** Grille,int *Tjoue);
//Selection morpion
...
void click_vide(int y,int x,int **Grille,SDL_Renderer *renderer, SDL_Window *window,int *Tjoue);
/*PROTOTYPES PENDU*/
		    							...
		    							if(first_loop == 0){
				    						...
				    						switch(event1.type){
				    							...
				    							case SDL_MOUSEBUTTONDOWN:
													printf("Tjoue : %d\n",Tjoue);						//clic de la souris

													
				    								click_morpion(renderer, window, &event1,0,Grille, &Tjoue);
...
void click_vide(int y,int x,int **Grille,SDL_Renderer *renderer, SDL_Window *window,int *Tjoue){
    ...
    if(*Tjoue==0){ /* tour joueur */
       Grille[y][x]= 1; /* croix */
       afficher_symbole(renderer,window, y ,x, Tjoue);
       ...
       // afficher_grille(Grille);
        verification(y,x,Grille,renderer,window, Tjoue);
    }
...
void verification(int y,int x,int **Grille,SDL_Renderer *renderer, SDL_Window *window,int* Tjoue){
	afficher_grille(Grille);
     printf(" y = %d x = %d\n",y,x);
    int a =3;
    int i,j,k,z;
    ...
    if (testGrilleremplie(Grille) == 1 ){
        ...
        if(*Tjoue== 0 && end != 1){
            ...
            *Tjoue = 1;
            ...
            ordiJoue(x,y,lignegagn,lignedef,Grille,renderer,window,Tjoue);
        }
...
void ordiJoue(int X, int Y, char lignegagn , char lignedef, int ** Grille,SDL_Renderer *renderer, SDL_Window *window,int *Tjoue){
    ...
    if(lignegagn != NULL){
            ...
            case 'h' :
                ...
                for(i=0;i<3;i++){ /* horizontale */
                    if(Grille[Y][i]== 0){
                        ...
                        click_vide(Y,i,Grille,renderer,window,Tjoue);
                    }
                }
            break;
            case 'v':
                ...
                for(i=0;i<3;i++){ /* verticale */
                    if(Grille[i][X]== 0){
                        ...
                        click_vide(i,X,Grille,renderer,window,Tjoue);
                    }
                }
            break;
            case 'd':
                ...
                for(i=0;i<3;i++){ /* diagonale descendande */
                    if(Grille[i][i]== 0){
                        ...
                        click_vide(i,i,Grille,renderer,window,Tjoue);
                    }
                }
            break;
            case 'D':
                ...
                for(i=3;i>0;i--){ /* diagonale ascendante */
                    if(Grille[i-1][3-i]== 0){
                        ...
                        click_vide(p,q,Grille,renderer,window,Tjoue);
                    }
                }
            break;
        }
        ...
        while(try == 1){
            ...
            if( verifierCaseValide(Grille,Y,X) == 1){
                try = 0;
                click_vide(Y,X,Grille,renderer,window,Tjoue);
            }
        }
    }
}
void croix_morpion(SDL_Renderer *renderer, SDL_Window *window, SDL_Event *event1, int restart){/* permet le reload de la grille de morpion */
...
void click_morpion(SDL_Renderer *renderer, SDL_Window *window, SDL_Event *event1, int restart, int ** Grille,int * Tjoue){
	...
	printf("Tjoue = %d \n",*Tjoue);
		...
		if( verifierCaseValide(Grille,Ya,Xa) == 1){
			...
			click_vide(Ya,Xa,Grille,renderer,window,Tjoue);
	...
	if(restart){
		...
		clear_image(NULL, NULL, texture_croix);
		clear_image(NULL, NULL, texture_rond);
		printf("clear croix\n");
	}
...
void afficher_symbole(SDL_Renderer *renderer, SDL_Window *window,int posy , int posx, int* Tjoue){
